...
function Signup() {
    ...
    let [selectedImage,setSelectedImage] = useState("./image/no image.jpg");
    ...
    let signupAccount = () => {
        if (stateSelectRef.current.value === "Select State") {
            ...
            resultParaRef.current.innerHTML = `${salutation} ${firstNameInputRef.current.value} ${lastNameInputRef.current.value} you know ${languageKnown.telugu ? "Telugu" : ""}${languageKnown.hindi ? ", Hindi" : ""}${languageKnown.english ? ", English" : ""} languages from ${capital}, ${stateSelectRef.current.value}. Your account has been created.`;
        }
    };
    ...
    return (
        <div className='App'>
            <form>
                ...
                <div>
                    <img className='pic' src={selectedImage}></img> 
                </div>
                <button
                    type='button'
                    onClick={() => {
